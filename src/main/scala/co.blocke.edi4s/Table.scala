...
case class Table(
  ...
  private def renderRow(cells: List[String], useColor: Boolean = false, headers: Boolean = false): String = {
    ...
    var colIdx = 0
    var inRow = true
    val isCollapsedRow = cells.exists(cell => cell.startsWith("~") || cell.startsWith("&"))
    val isHighlightedRow = cells.exists(cell => cell.startsWith("!") || cell.startsWith("&"))
    ...
    while (colIdx < cells.length && inRow) {
      ...
      if (raw.startsWith("~") || raw.startsWith("&")) {
        ...
        val content = raw.stripPrefix("~").stripPrefix("&")
        val color = if (useColor && isHighlightedRow) BLUE else ""
        sb.append("|").append(pad(s"$color$content$RESET", collapsedWidth)).append("|")
        inRow = false
      } else {
        val indentLevel = raw.takeWhile(_ == '>').length
        val content = raw.dropWhile(_ == '>').stripPrefix("!").stripPrefix("~").stripPrefix("&")
        val indented = ("  " * indentLevel) + content
        val color =
          if (useColor && headers) BOLD + RED
          else if (useColor && isHighlightedRow) BLUE
          else ""
  ...
  def writeXls(path: String): Unit = {
    ...
    for (row <- rows) {
      val collapseIdx = row.indexWhere(cell => cell.startsWith("~") || cell.startsWith("&"))
      val r = sheet.createRow(rowIdx)
      r.setHeightInPoints(20)

      if (collapseIdx >= 0) {
        val before = row.take(collapseIdx).map(_.replaceAll(">", "").stripPrefix("!").stripPrefix("~").stripPrefix("&"))
        ...
        val collapseText = row(collapseIdx).stripPrefix("~").stripPrefix("&")
        ...
        row.map(_.replaceAll(">", "").stripPrefix("!").stripPrefix("~").stripPrefix("&")).zipWithIndex.foreach { case (v, i) =>
          val c = r.createCell(i)
          c.setCellValue(v)
  ...
  private def toPlainText(useColor: Boolean): String = {
    ...
    while (i < rowCount) {
      val row = rows(i)
      val isCollapsed = row.exists(cell=> cell.startsWith("~") || cell.startsWith("&"))
      val isHighlighted = row.exists(cell=> cell.startsWith("!") || cell.startsWith("&"))
      val prevHighlighted = i > 0 && rows(i - 1).exists(cell=> cell.startsWith("!") || cell.startsWith("&"))

      if (isHighlighted && !prevHighlighted && i > 0) {
        sb.append(renderDivider('-')).append("\n")
      }
    ...
    val lastHighlighted = rows.nonEmpty && rows.last.exists(cell=> cell.startsWith("!") || cell.startsWith("&"))
    if (!lastHighlighted)
